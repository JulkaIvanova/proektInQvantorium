...
class CinemaHall:
    ...
    def book(self, time=None, move=None) -> None:
        ...
        while  row > self.column or seat > self.row or row <= 0 or seat <= 0 or (move, seat, row, time) in self.bookedSeates:
...
def createHall() -> CinemaHall:
    return CinemaHall(int(input("Введите количество кресел в ряду: ")), int(input("Введите количество рядов: ")), input("Введите название зала: "), dict())
...
class SystemOfCinema:
    ...
    def book(self) -> None:
        target = input("Получить помощь системы в подборе кинотеатра(1)\nВыбрать кинотеатр самостоятельно(любой другой сивол)\n")
        if target == "1":
            b = []
            cinemas_names = set()
            films = set()
...
move = Movies("Майор Гром: Игра", 130)
# print(move)
a = [Movies("Майор Гром: Трудное детсво", 100), Movies("Преступление и наказание", 160), Movies("Аватар2", 180)]
b = [CinemaHall(5, 10, "A", {"09:00":a[0], "11:00":a[1], "14:00":a[2]}), CinemaHall(4, 6, "B", {"10:00":a[0], "13:00":a[1], "19:00":a[2]})]
k = [CinemaHall(20, 10, "A", {"09:00":a[2], "11:00":a[0], "14:00":a[1]}), CinemaHall(4, 6, "B", {"10:00":a[1], "13:00":a[1], "19:00":a[1]})]
# print(b[0])
# print(Cinema("Ультрасинема", b))
# b[0].book()
# b[0].book()
# b[0].book()
# b[0].book()
# b[0].book()
u = SystemOfCinema([Cinema("Ультрасинема", b), Cinema("Кинотеатр Россия", k)], "РосКино")
# u.append()
for i in range(10):
    u.book()
