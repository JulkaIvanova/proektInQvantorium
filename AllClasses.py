...
class CinemaHall:
    ...
    def book(self, message, bot, time=None, move=None) -> None:
        ...
        if time == None and move == None:
            ...
            bot.send_message(chat_id=message.chat.id, text="Выберите время: ", reply_markup=moveKeybord)
            time = message.text

            while not (time in times):
                bot.send_message(chat_id=message.chat.id, text="Сегодня в это время фильм не показывают. Выберите время: ", reply_markup=moveKeybord)
                time = message.text
...
class Cinema:
    ...
    def book(self, bot, message, menuKeyboard) -> None:
        ...
        if target == "Да":
            ...
            keyboard.add(*d)
            bot.send_message(chat_id=message.chat.id, text='Выберите интересующий вас фильм:',
                         reply_markup=keyboard)
            target1 = message.text

            while not (target1 in films):
                bot.send_message(chat_id=message.chat.id, text='Такой фильм сегодня не показывают. Выберите интересующий вас фильм:',
                                 reply_markup=keyboard)
                target1 = message.text
            halls = set()
            string1 = ""
            ...
            while not (hall in hallss):
                bot.send_message(chat_id=message.chat.id, text="Такого кинозала не сушествует. Введите название кинозала: ", reply_markup=keyboard4)
                hallss = message.text
