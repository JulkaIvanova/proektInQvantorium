...
class Cinema:
    ...
    def get_halls(self, film):
        _, bfffffffffffffff = self.get_films()
        halls = set()
        string1 = ""

        for i in bfffffffffffffff:
            if i[1].name == film:
                halls.add(i[0].name)
                minutes = int(i[2].split(":")[1])
                houers = int(i[2].split(":")[0])
                minutes = minutes + i[1].during % 60

                if len(str(minutes)) == 1:
                    minutes = str(minutes).replace(str(minutes), "0" + str(minutes))
                string1 += (
                    f"Кинотеатр: {self.name}\nЗал: {i[0].name}\nВремя:{i[2]}-{houers + i[1].during // 60}:{minutes}\n\n")
        return (halls, string1)
    
    def get_times(self, film, hall):
        _, bfffffffffffffff = self.get_films()
        time = set()
        string2 = ""

        for i in bfffffffffffffff:
            if i[1].name == film and i[0].name == hall:
                time.add(i[2])
                minutes = int(i[2].split(":")[1])
    ...
    def book(self, bot:telebot.TeleBot, message, menuKeyboard) -> None:
        global value
        def book2(message):
            ...
            if message.text == "Получить помощь":
                ...
                #for i in self.halls:
                #    for key, value in i.moviesAndTime.items():
                #        bfffffffffffffff.append((i, value, key))
                #        films.add(value.name)
                films, bfffffffffffffff = self.get_films()
                d = []
                ...
                target1 = message.text  # тут message.text все еще "Получить помощь"

                while not (target1 in films): # А тут запускается бесконечный цикл
